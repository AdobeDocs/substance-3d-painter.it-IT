...
È ora possibile **esportare la vista 2D** eseguendo il rendering di **come texture**. Questa funzione è stata richiesta da molti utenti e finalmente è stata resa disponibile! Il processo di esportazione assumerà lo stato corrente della **vista 2D** per eseguire il rendering di una texture con le normali impostazioni di esportazione (spaziatura interna, formato del file, profondità di bit). Ciò significa che se la modalità di visualizzazione è impostata su **Solo** invece della modalità **Materiale**, la visualizzazione 2D verrà esportata così com&#39;è.

Passa alla **finestra Esporta** e scegli la nuova configurazione denominata &quot;**Vista 2D**&quot;:\
...
### Filtro luce al forno migliorato
...
In questa nuova versione viene introdotto un nuovo shader denominato &quot;**pbr-metal-rough-anisotropia-angle**&quot;. Questo shader supporta due canali denominati &quot;**Angolo Anisotropia**&quot; e &quot;**Livello Anisotropia**&quot; che possono essere utilizzati per creare riflessi di specular anisotropi. Questo shader si tradurrà anche in Iray as-is senza la necessità di alcuna conversione.

È possibile accedere al nuovo shader tramite la [finestra shader](../../interface/shader-settings/shader-settings.md) facendo clic sul pulsante shader e aprendo il mini-shelf:
...
Per comodità, è stato aggiunto anche un nuovo modello di progetto pronto per la creazione di texture per questo nuovo shader denominato: **PBR - Metallic Roughness Coated**.
...
Questa nuova tecnica offre risultati molto migliori in ogni caso per un costo molto minimo. **Il TAA** funziona accumulando informazioni su più fotogrammi, consentendo di produrre bordi molto uniformi senza perdere dettagli.
...
Questo nuovo sistema cambia alcuni fondamenti della Substance Painter e il modo in cui funziona l&#39;applicazione. Substance Painter ora utilizza il file SVT per mantenere uno specifico ingombro di memoria per la finestra della vista, consentendo lo **streaming delle texture in entrata e in uscita**. Il vantaggio principale è la possibilità di caricare progetti più grandi con maggiore facilità e ridurre la pressione sulla GPU per **migliorare le prestazioni**. Ciò significa che se le dimensioni iniziano a diventare eccessive, alcune texture verranno scaricate sul disco e recuperate in un secondo momento, se necessario. Questa è una **cache volatile** che viene eliminata alla chiusura dell&#39;applicazione.

Un altro vantaggio del sistema è l&#39;introduzione di **mipmap** all&#39;interno del **viewport** che migliorerà la qualità delle texture e ridurrà l&#39;effetto Moire particolarmente visibile con i pattern Fabric.
...
* **Accelerazione supporto hardware**: se abilitata, Substance Painter utilizzerà il supporto nativo delle texture sparse da parte della GPU (se disabilitata, verrà eseguito il fallback su un&#39;implementazione software)
...
Lo strumento di simmetria è stato rielaborato e ora consente di spostare il punto di origine. Quando un progetto è parzialmente simmetrico o decentrato, il piano può ora essere regolato. L’offset verrà salvato all’interno del progetto per asse.

Abbiamo anche colto l&#39;occasione per amare la funzionalità e ottenere ora un nuovo feedback visivo:

* Una **linea di intersezione** è ora disegnata da **default** sulla trama per mostrare dove si trova il piano di simmetria.
...
* **Piano di simmetria**: Mostra piano consente di disegnare un piano che taglia la trama. Mostra intersezione disegna una linea sulla trama in cui il piano taglia la trama.
* **Cursore simmetria** :Show Il cursore disegnerà un cursore pennello secondario in cui viene applicata la simmetria. Nascondi durante disegno visualizza il cursore solo quando non si disegna.
* **Manipolatore**: Mostra manipolatore visualizza un manipolatore nella finestra della vista per spostare il piano di simmetria. **La dimensione del manipolatore** controlla la dimensione del controller nella finestra della vista.

Per nascondere/mostrare il manipolatore di simmetria è possibile utilizzare le stesse **scelte rapide** utilizzate per il manipolatore Tri-Planar e UV:
...
### Manipolatore triplanare migliorato

![](../../assets/trip-rotation-optim.gif)

Oltre ai 3 assi originali per il controllo della rotazione, abbiamo anche aggiunto una nuova sfera di rotazione per il controllo del manipolatore triplanare. La sfera consente di provare rapidamente diverse angolazioni durante la proiezione dei pattern di disturbo, ad esempio.

### Esportazione di texture con dithering a 8 bit

![](../../assets/dither-1.jpg)

Quando si esportano texture di mappe Normali e di Height in formati di file in modalità 8 bit, ora la Substance Painter applicherà automaticamente il **dithering** per ridurre **banding** **problemi**.
...
Sono stati apportati alcuni miglioramenti al flusso di lavoro per quanto riguarda la gestione dello stack di livelli e la gestione dei livelli:

* Assegna **colore** a **livelli** e **cartelle** all&#39;interno della pila di livelli tramite il menu **clic con il pulsante destro del mouse** per organizzare i livelli.\
  ...
  * Se un livello ha un colore dedicato, non verrà sovrascritto dalla cartella.Questo comportamento originale semplifica la colorazione e l’organizzazione del gruppo di livelli senza dover assegnare troppi colori a mano.
...
* **Passate rapidamente da un metodo di fusione all&#39;altro** con le **scelte rapide da tastiera** della **freccia**.\
  Dopo la **chiusura** del menu a comparsa per la fusione, l&#39;**elemento attivo** **rimarrà** sul livello che può essere modificato con la stessa scelta rapida precedente.
...
Ad esempio, ora forniamo un nuovo **generatore di maschere** denominato &quot;**Distanza bordo UV**&quot; in cui viene creata una maschera in bianco e nero dal bordo delle Isole UV del set di texture corrente.

![](../../assets/uv-border.png)

>[!NOTE]
>
> Questi input vengono forniti direttamente dal motore di Substance Painter in base al progetto Trama e non utilizzano i [fornai](../../baking/baking.md).
...
* Nuovi **pattern sfumatura** procedurali da utilizzare con il nuovo shader **anisotropo**:
...
* Nuovo **progetto** **modelli**:
  ...
  * PBR - Rugosità metallica patinata
* Nuovo **materiale**:

  * Uomo Femmina 30s Face 06 (si può trovare rapidamente tramite il predefinito Pelle nello scaffale)\
    Questo nuovo materiale della pelle è stato fornito da **Texturing.XYZ** e offre ottimi dettagli della superficie per dipingere una pelle realistica.\
...
* Aggiornamento di **Progetto di esempio**: la sfera di anteprima può ora essere utilizzata con simmetria e ha un nuovo angolo della videocamera per rendering personalizzati. Lo shader predefinito è ora &quot;Angolo di Anisotropia&quot;.
