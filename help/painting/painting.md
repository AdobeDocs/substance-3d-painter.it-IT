---
helpx_url: "https://helpx.adobe.com/it/substance-3d-painter/painting.html"
breadcrumb-title: ''
description: Scoprite come colorare le texture direttamente sui modelli 3D in Substance 3D Painter utilizzando vari pennelli, strumenti e tecniche.
helpx_creative_field: ""
helpx_description: Painter > Painting
helpx_experience_level: ""
helpx_learn_topic: ""
helpx_tags: ""
title: Pittura
user-guide-description: ''
user-guide-title: ''
source-git-commit: 9f20406f682e0e6a2e9a423e81c5ecfc7430ecfd
workflow-type: tm+mt
source-wordcount: '99'
ht-degree: 8%

---


# Pittura

La pittura in Substance 3D Painter può essere realizzata in molti modi con strumenti manuali o azioni automatiche.

Gli strumenti di pittura manuale sono accessibili direttamente tramite la barra degli strumenti (o la relativa scelta rapida da tastiera). Quando è selezionato uno strumento di disegno, le sue proprietà vengono visualizzate nella finestra Proprietà.

Per informazioni su ogni strumento, consultate la relativa pagina dedicata:

* [Elenco degli strumenti di pittura](../painting/tool-list/tool-list.md)
* [Elenco degli strumenti tracciato](tool-list/path.md)
* [Linea retta](straight-line.md)
* [Mouse lento](lazy-mouse.md)
* [Simmetria](../painting/symmetry/symmetry.md)
* [Riempi proiezioni](../painting/fill-projections/fill-projections.md)
* [Predefiniti](../painting/presets/presets.md)
* [Tratti dinamici](../painting/dynamic-strokes/dynamic-strokes.md)
* [Colorazione avanzata del canale](../painting/advanced-channel-painting/advanced-channel-painting.md)
* [Grafica vettoriale (.svg e .ai)](vector-graphic-svg.md)
* [Risorsa di testo](text-resource.md)
