---
helpx_url: "https://helpx.adobe.com/it/substance-3d-painter/release-notes/old-versions/version-2021-1-7-1-0.html"
breadcrumb-title: ''
...
**Substance Painter 2021.1 (7.1.0)** introduce diverse nuove funzioni e miglioramenti, quali la maschera di geometria e la copia e incolla degli effetti nella Pila livelli.
...
La maschera Geometria è un nuovo strumento di mascheratura nella Pila livelli che consente di nascondere la geometria in base ai nomi di trama o alle Porzioni UV. Si tratta di un&#39;evoluzione della maschera per porzioni UV precedentemente denominata che ha mascherato la geometria in base ai numeri UDIM.

Questo nuovo strumento è un modo migliore di mascherare la geometria rispetto alla pittura regolare (o quando si utilizza [Riempimento poligonale](../../painting/tool-list/polygon-fill.md)) perché beneficia di diverse ottimizzazioni del motore. Inoltre, non è distruttivo in quanto non memorizza informazioni sulla geometria (come facce o vertici) ma solo il nome della trama o il numero di porzione UV, quindi la reimportazione di una trama non interromperà la maschera. Un altro vantaggio consiste nel fatto che, nascondendo la geometria, è possibile visualizzare pitture su superfici non accessibili in precedenza all&#39;interno di un insieme di texture. In questo modo si evita, ad esempio, di dividere un oggetto in più insiemi di texture.
...
* **Colorazione della geometria nascosta/non raggiungibile.**\
  Dopo aver selezionato la geometria da mascherare nella maschera Geometria, è possibile attivare il pulsante **Nascondi/Ignora geometria esclusa** nella parte superiore della finestra della vista (o premendo la scelta rapida da tastiera **ALT+H**). Quando questa opzione è attivata, la geometria esclusa viene nascosta (come altri insiemi di texture) per mostrare solo la geometria inclusa o colorabile con il livello corrente. Questa opzione consente di pittura le aree precedentemente bloccate o fuori portata. Questa opzione si applica anche a qualsiasi tipo di livello.
...
### Nuova copia e incolla degli effetti Pila livelli

![](../../assets/header-effects.jpg)

Ora potete copiare gli effetti su più livelli e Pile livelli, come avviene per i livelli normali.
...
* **Copiare e incollare tramite il menu di scelta rapida**\
  Fate clic con il pulsante destro del mouse su un effetto qualsiasi nella Pila livelli di un set di texture e scegliete l’azione Taglia o Copia. Quindi fate nuovamente clic con il pulsante destro del mouse su un livello e scegliete incolla per spostare o creare una copia degli effetti desiderati. Abbiamo anche colto l&#39;occasione per rielaborare il menu di scelta rapida e dare accesso a più funzionalità:

  ![](../../assets/effects-contextual-menu.png)

* **Copiare e incollare con le scelte rapide da tastiera**\
  Come per qualsiasi livello, è possibile utilizzare le scelte rapide da tastiera da tastiera **CTRL+C** (copia)/**CTRL+X** (taglio) e **CTRL+V** (incolla) per copiare gli effetti in base alla selezione corrente. Come per i livelli, gli effetti vengono inseriti sopra la selezione corrente.
...
* **Aggiungi una descrizione per Porzione UV**\
  È ora possibile aggiungere una descrizione per ogni Porzione UV tramite l’elenco Set di texture. Ciò semplifica la navigazione del progetto, soprattutto durante l’esportazione e la esegue i baking, poiché le descrizioni possono essere visualizzate anche in questi contesti.\
  Per aggiungere o modificare una descrizione, è sufficiente fare clic su una Porzione UV nella finestra Elenco set di texture e quindi passare alla finestra Impostazioni set di texture per modificarla.
...
* **Nuove miniature di Pila livelli**\
  Le miniature delle Pile livelli ottimizzate sono state migliorate. Ora viene visualizzata una sfera di materiale per i livelli di riempimento, per facilitare la navigazione e visualizzare le proprietà principali di ogni livello anche quando si lavora con il flusso di lavoro [Porzione UV](../../features/uv-tiles/uv-tiles.md). La miniatura viene generata dalle informazioni sul livello, ma non tiene conto degli effetti per evitare di essere ricalcolata troppo spesso.

  ![](../../assets/layer-stack-thumbnails.png)

* **Uscita della maschera di geometria migliorata nella Pila livelli**\
  L&#39;uscita dalla maschera di geometria (in precedenza maschera di Porzione UV) potrebbe risultare difficile con le cartelle nella Pila livelli se questa non era dotata di maschera. Questo perché non c&#39;era altro contesto da attivare se non la selezione di un altro livello. È ora possibile fare clic sulla miniatura della cartella per uscire dalla maschera Geometria. È inoltre possibile trascinare materiali o materiali avanzati dallo scaffale nella finestra della vista durante la modifica della maschera di geometria.
...
* **Nuovo pulsante esegue i baking set di texture corrente**\
  Un nuovo pulsante è stato aggiunto nella parte inferiore della finestra Cottura, per facilitare e velocizzare la ripetizione di un set di texture. L’uso di questo pulsante non influisce sulla selezione personalizzata definita in precedenza e esegue i baking invece l’intero set di texture (comprese tutte le sue Porzioni UV, se disponibili).
...
Per ulteriori dettagli sulle nuove funzionalità di Substance Engine, consulta [questa pagina della documentazione](https://helpx.adobe.com/it/substance-3d/unlisted/documentation/sddoc/version-2020-2-10-2-0-200574252.html).
...
Si consiglia di attivare la funzione [Mouse lento](../../painting/lazy-mouse.md) nella barra degli strumenti contestuale per migliorare la qualità dei punti colorati.
...
* **Informazioni sulla Porzione UV**\
...
Riepilogo: **Versione principale, nuova maschera di geometria che consente di selezionare e pittura parti della geometria, copiare/incollare effetti nella Pila livelli, miglioramento del flusso di lavoro della Porzione UV, aggiornamento di Iray, Baker, Substance Engine e nuovo contenuto**

**Aggiunto:**

* Nuova maschera di geometria e pittura parti selezionate della geometria
* [Maschera geometria] Consente di pittura le parti selezionate della geometria in base ai nomi della trama
