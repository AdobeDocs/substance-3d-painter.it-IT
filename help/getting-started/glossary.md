...
| **Alpha** | Un canale alfa è una maschera che può essere utilizzata per pittura dettagli o forme complesse, ad esempio un codice a barre o un logo. |
| **Esegue i baking** | Per &quot;infornata&quot; si intende l’azione di calcolare le informazioni da una trama 3D e salvarle in una texture basata sulle informazioni UV di una trama. |
| **Profondità di bit** | La profondità di bit è la quantità di informazioni che può essere memorizzata in una texture (per colore). Maggiore è il numero, migliore è la precisione delle informazioni. Tuttavia le prestazioni diminuiscono con numeri elevati durante l&#39;esecuzione dei calcoli. |
...
| **Clona /Clone** | Il clone è uno strumento utilizzato per replicare parte del quadro/della texture in un’altra posizione. |
...
| **Esporta** | L’azione di esportazione è il modo di generare texture appiattite dal dipinto realizzato all’interno dell’applicazione. Le texture create dall’esportazione possono essere utilizzate in altre applicazioni. |
...
| **Filtraggio** | I filtri si riferiscono alla modalità di visualizzazione della texture all&#39;interno di una finestra della vista 3D. I più comuni sono i più vicini (i pixel vengono letti così com’è, creando un’immagine apparentemente squadrata verso l’alto) e bilineari (i pixel vengono interpolati, creando un’immagine sfocata verso l’alto). |
| **GPU** | La GPU (Graphic Processing Unit) è la parte di un computer che esegue calcoli rapidi per la produzione di immagini. |
| **Generatore** | Un Generatore è una Substance che genera nuove informazioni/immagini in genere basate su texture aggiuntive. Alcuni generatori di maschere, ad esempio, usano la texture eseguita i baking per creare maschere complesse. |
...
| **Pila livelli** | Una Pila livelli è un luogo in cui i livelli possono essere gestiti e organizzati. I livelli sono organizzati dal basso verso l’alto. Il livello inferiore verrà disegnato per primo, quindi ogni livello superiore verrà aggiunto uno alla volta l&#39;uno sopra l&#39;altro. |
| **Mouse lento** | Il Mouse lento è un comportamento dello strumento pennello. Rallenta il tracciato del pennello per migliorare la precisione durante il disegno e crea un ritardo/scostamento tra il cursore del mouse e il disegno effettivo. |
...
| **Mappa trama** | Una mappa di trama è una mappa eseguita i baking da una trama che contiene informazioni relative alla trama stessa. Può essere un&#39;informazione di posizione o un&#39;informazione di occlusione, ad esempio. |
| **Mip-Map** | Mip-map è una texture precalcolata, di solito presente come sequenza di immagini ogni volta a una risoluzione inferiore rispetto alla texture originale. |
...
| **Ortografica** | Una proiezione Ortografica è un mezzo per rappresentare oggetti tridimensionali in due dimensioni in cui tutte le linee di proiezione sono ortogonali al piano di proiezione. |
...
| **Prospettiva** | La Prospettiva è una rappresentazione approssimativa o un oggetto o una scena vista dall&#39;occhio umano su una superficie piana (come uno schermo). È una simulazione di profondità e scala. |
...
| **Procedurale** | Procedurale è un termine che descrive un processo generato da un computer in base a una serie di parametri. Può trattarsi semplicemente di risultati matematici come numeri o immagini complesse. |
...
| **Maschera avanzata** | Le maschere avanzate si comportano come materiali avanzati, ma invece di essere livelli sono effetti definiti per generare maschere solo in base alla trama 3D corrente. |
| **Materiale avanzato** | Un materiale avanzato è un gruppo di livelli salvati come un unico file. Materiale avanzato può adattarsi a ogni progetto in Substance 3D Painter, consentendo di creare materiali che cambieranno a seconda della trama 3D corrente. |
...
| **Stencil** | Uno stencil è un&#39;immagine allineata allo schermo e utilizzata con una proiezione della fotocamera per pittura sulla trama 3D. |
| **Substance** | Una Substance è un formato di file che consente di generare una texture in base a un insieme di parametri (che implicano calcoli procedurali). Questi parametri possono essere modificati per creare variazioni. |
| **Simmetria** | La simmetria è un&#39;opzione di uno strumento che consente di pittura in due punti contemporaneamente in modo speculare. |
| **Modello** | Un modello è un insieme di opzioni predefinite utilizzate durante la creazione di un nuovo progetto. Ad esempio, può definire la risoluzione predefinita o l’impostazione di esegue i baking predefinita. |
...
| **Lavorazione** | L&#39;aratura è la ripetizione di una texture in cui le cuciture non sono visibili ai bordi, è intesa a simulare un piano infinito. Esempio: erba o marciapiedi. |
| **Strumento** | Uno strumento si riferisce a un’azione che consente di interagire con la trama 3D, spesso per pittura o applicare effetti. |
...
| **UDIM** | L’UDIM consente di dividere gli UV di una trama 3D su più intervalli per aumentare la risoluzione generale della texture. |
| **UV** | I raggi UV sono informazioni definite su una trama 3D che indicano come può essere dispiegata per diventare una forma piatta. Queste informazioni vengono utilizzate per proiettare una texture su una trama 3D. Substance 3D Painter consente la pittura solo nell’intervallo 0-1, che rappresenta le dimensioni di una texture. Altri intervalli sono supportati solo tramite il sistema UDIM. |
