...
La maschera Geometria è una maschera secondaria sui livelli che consente di mascherare un livello in base alla geometria del modello 3D dell’insieme di texture associato. Può essere mascherata in base ai nomi delle trame o ai riquadri UV.
...
La maschera Geometria (Geometry) è uno strumento utile per eliminare rapidamente gran parte della geometria del modello 3D. Offre diversi vantaggi alla maschera di pittura:
...
* Come per le maschere di disegno, la maschera di geometria può essere applicata a un gruppo per agire su più livelli contemporaneamente.
...
| <div><img class="" data-preserve-html="true" id="root_content_flex_items_position_position-par_dx_table_row-r3-column-c0_dynamic_grid_items_grid-cell_position-par_image" src="../../assets/geo-mask-icon-uvtiles.png"/></div> | Uno o più riquadri UV sono stati esclusi. Il numero indica la quantità di elementi rimanenti ancora interessati dal livello. |
...
Per modificare la maschera Geometria di un determinato livello, fate clic sull’icona dedicata. Per uscire dalla modalità di modifica, è sufficiente fare clic su un’altra parte del livello, ad esempio il contenuto o la maschera di disegno:
...
| Tipo | Descrizione |
| --- | --- |
| **Riquadri UV** | La mascheratura viene eseguita specificando il numero di porzioni UV (UDIM) da includere. Questo è il metodo più performante che permette di scartare completamente una texture da essere calcolata. |
...
### Azioni stack di livelli

![](../../assets/geo-mask-actions.png)

Lo stato della maschera Geometria può essere modificato rapidamente dalla pila di livelli facendo clic con il pulsante destro del mouse sull’icona.
...
Quando parti della geometria sono state escluse, possono essere nascoste nella finestra della vista. Ciò consente di disegnare sulla geometria che era precedentemente sotto e non accessibile.
...
Nell’esempio seguente, il modello 3D è stato suddiviso in due oggetti: una parte superiore e una inferiore. Per impostazione predefinita, i tratti del pennello entrano in collisione con tutti gli oggetti. escludendo la parte superiore è ora possibile colorare esclusivamente la parte inferiore.

>[!NOTE]
>
> L’elenco delle maschere di inclusione/esclusione della geometria è dinamico e la modifica del suo stato attiverà un nuovo calcolo dei tratti di pennello nel livello. Ciò consente di regolare la mascheratura senza perdere i tratti del pennello quando si reimporta una trama con nuove porzioni UV o se i nomi della trama sono cambiati. Tuttavia significa anche che i tratti del pennello non sono cotti al forno, quindi qualsiasi modifica della maschera di geometria potrebbe in seguito causare una proiezione errata del pennello.

| Visivo | Descrizione |
| --- | --- |
| <div><img class="" data-preserve-html="true" id="root_content_flex_items_position_position-par_dx_table1_row-r1-column-c0_dynamic_grid_items_grid-cell_position-par_image" src="../../assets/no-geo-excluded.jpg" width="420px"/></div> | Nella maschera Geometria non è stata esclusa alcuna geometria; il livello di disegno in cui è stato eseguito il tratto del pennello bianco entra in collisione con tutta la geometria.Il pulsante **Nascondi geometria esclusa** è disabilitato. |
