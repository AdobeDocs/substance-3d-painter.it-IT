...
Non esiste un benchmark specifico per il conteggio multiplo che Painter è in grado di gestire, in quanto dipende in gran parte dalle specifiche della macchina, dall&#39;assegnazione del set di texture e dalle proprietà della Pila livelli, ma meno di 10 milioni di poly devono essere gestiti correttamente se si tiene conto delle ottimizzazioni della Pila livelli.

+++

+++Modelli con basso numero di poligoni
C&#39;è una cosa come il poly troppo basso. Questo perché il motore di texture utilizza i poligoni per sapere quale parte della trama deve essere sottoposta a rendering per calcolare i tratti del pennello. Le trame con un numero di poligoni molto basso possono essere riprodotte completamente anche con tratti di pennello piccoli che possono sovraccaricare inutilmente la GPU.
...
È preferibile dividere trame più grandi con assegnazioni di materiale più complesse in diversi insiemi di texture. I set di texture consentono di assegnare diverse impostazioni per set di texture, come la risoluzione e le proprietà di shader. Se, ad esempio, solo una parte della trama utilizza traslucidità o SSS, è preferibile assegnare un altro insieme di texture e un’istanza shader diversa alla parte. In questo modo, non è necessario calcolare queste proprietà più complesse dove non vengono utilizzate.

+++

+++Mantieni le Isole UV vicine
Provate a mantenere vicine Isole UV vicine in uno spazio 3D. Questo vale sia per il layout UDIM che per il layout dello spazio UV classico. Se hanno tratti di pittura o texture condivisi, è più facile calcolarli quando sono raggruppati nella stessa area dello spazio UV, piuttosto che se si trovano a estremità opposte.

Il motore a texture funziona dividendo una texture in pezzi più piccoli per velocizzare il calcolo. Ciò significa che ogni tratto aggiorna solo i pezzi che devono essere modificati, invece di aggiornare l’intera texture con ogni tratto. Mantenendo le Isole UV adiacenti vicine l&#39;una all&#39;altra, riduce al minimo il numero di blocchi interessati da un singolo tratto.
