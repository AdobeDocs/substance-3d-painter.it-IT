...
<b>Il pannello delle impostazioni della mappa trama</b> è disponibile in modalità di Esegue i baking e dispone dei controlli per preparare la trama per la esegue i baking. Per regolare le impostazioni della mappa trama per una determinata mappa, selezionate la mappa nel <b>pannello baker mappa trama</b>. Ogni mappa trama può avere diverse impostazioni disponibili. Nella parte superiore del pannello Baker di mappe trama è disponibile una raccolta di <b>impostazioni comuni </b>condivise da tutte le mappe trama.
...
La pagina delle impostazioni comuni contiene i controlli che influiscono sul modo in cui vengono eseguite i baking tutte le mappe trama.
...
| Larghezza dilatazione | Regolate l’estensione delle informazioni eseguite i baking oltre i limiti delle Isole UV. |
...
| Impostazione | Funzione |
| --- | --- |
| Usa trama poly bassa come trama poly alta | Abilita questa impostazione per eseguire i baking le mappe in base alla trama del progetto. |
...
| Gabbia | Determinare la modalità di generazione della gabbia eseguente i baking.<ul data-preserve-html="true"> <li data-preserve-html="true">Basato sulla distanza: gonfiate i vertici lontano dalla trama a una distanza uniforme attraverso il modello per creare una gabbia.</li> <li data-preserve-html="true">Automatico (sperimentale): Painter analizza la trama e genera automaticamente una gabbia, cercando di mantenere la gabbia vicina alla superficie senza creare intersezioni per risultati ottimali.</li> <li data-preserve-html="true">File personalizzato: importa un file creato per utilizzarlo come gabbia. I file importati devono avere lo stesso numero di vertici della trama di base per funzionare correttamente.</li> </ul> |
| Ignora backface | Attivate/disattivate se i volti posteriori vengono ignorati durante la esegue i baking. Questo può aiutare a ridurre gli artefatti, ma può anche causare errori in alcuni casi limite. |
| Corrispondenza | Modificate il modo in cui il Baker determina se includere gli oggetti durante la esegue i baking:<ul data-preserve-html="true"> <li data-preserve-html="true">Sempre: includi tutte le trame di poli elevate che vengono colpite all&#39;interno della gabbia durante la esegue i baking.</li> <li data-preserve-html="true">Per nome trama: per ogni gabbia, esegue i baking solo maglie con il suffisso di trama corrispondente.</li> </ul> |
...
| Anti-alias | Regola la quantità di anti-alias nelle mappe generate. |

#### Correzione dell&#39;inclinazione

| Impostazione | Descrizione |
| --- | --- |
| **Pittura correzione inclinazione** | Accedete alla modalità di disegno della correzione dell&#39;inclinazione. |
...
| Impostazione | Funzione |
| --- | --- |
| Origine colore | Modifica la modalità di determinazione dei colori eseguiti i baking della mappa ID:<ul data-preserve-html="true"> <li data-preserve-html="true">Colore vertice</li> <li data-preserve-html="true">Colore materiale</li> <li data-preserve-html="true">ID file</li> <li data-preserve-html="true">ID trama/Poligruppo</li> </ul> |
...
## Occlusione ambientale impostazioni mappa
...
| Occlusione autonoma | Selezionate le trame che devono influire sull’occlusione ambientale della trama corrente. |
...
| Raggio di campionamento | Regolate la distanza di ricerca del baker per calcolare la curvatura del punto corrente. |
...
## Impostazioni mappa di spessore

| Impostazione | Funzione |
| --- | --- |
| Raggi secondari | Modifica il numero di raggi secondari. Più raggi possono produrre risultati migliori a costo di tempi di elaborazione più lunghi. |
| Distanza minima occlusione | Regolate la distanza minima per i raggi da percorrere per colpire la geometria poly elevata e influire sulla mappa di spessore risultante. |
| Distanza massima occlusione | I raggi che si estendono oltre questa distanza senza toccare la maglia di poli superiore sono considerati come non occlusi e non influiscono sulla mappa di spessore. |
| Relativo al rettangolo di selezione | Quando questa casella è selezionata, altre impostazioni che fanno riferimento alla distanza sono basate sul rettangolo di selezione della trama del progetto. Quindi una distanza di 1 è uguale alla dimensione del rettangolo di selezione. |
| Angolo di diffusione | Regolate l’intervallo angular dei raggi generati. Un angolo di estensione più elevato consente di occludere più facilmente una superficie mediante una geometria che non è posizionata perpendicolarmente alla superficie. |
| Distribuzione | Seleziona la modalità di distribuzione dei raggi. |
...
## Impostazioni mappa altezza

| Impostazione | Funzione |
| --- | --- |
| Normalizzazione | Modificare la modalità di normalizzazione dei valori dei height. |
| Divisore di scala | Se Normalizzazione è impostata su Manuale, utilizzare questo cursore per regolare il divisore di ridimensionamento e la normalizzazione della mappa dell&#39;altezza. |

## Normali incurvate impostazioni mappa

| Impostazione | Funzione |
| --- | --- |
| Raggi secondari | Modifica il numero di raggi secondari. Più raggi possono produrre risultati migliori a costo di tempi di elaborazione più lunghi. |
| Distanza minima occlusione | Regolate la distanza minima per i raggi da percorrere per colpire la geometria poly elevata e influire sulla mappa normali incurvate risultante. |
| Distanza massima occlusione | I raggi che si estendono oltre questa distanza senza toccare la maglia di poli superiore sono considerati come non occlusi e non influiscono sulla mappa normali incurvate. |
