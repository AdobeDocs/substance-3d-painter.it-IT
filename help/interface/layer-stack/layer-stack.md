...
breadcrumb-title: ''
description: Scoprite come utilizzare la pila di livelli in Substance 3D Painter per organizzare e gestire più livelli di pittura di texture.
...
La **pila di livelli** consente di manipolare i livelli di un set di texture. Un livello contiene il disegno e gli effetti che creeranno la texture sull’oggetto 3D nella scena. Potete nascondere e scoprire i livelli, inserirli in cartelle e modificarne l’opacità e il metodo di fusione.
...
* **Livello pittura**: questo tipo di livello può essere colorato con pennelli e particelle
...
* **Cartella**: questo tipo di livello ha solo lo scopo di contenere altri livelli, viene utilizzato principalmente per organizzare la pila di livelli
...
Potete colorare la maschera manualmente (in scala di grigio con un pennello) oppure utilizzare filtri e sostanze per ottenere risultati più dinamici/procedurali.

## Modalità visualizzazione

![](../../assets/switch-viewmode-optim.gif)

Il menu a discesa in alto a sinistra della pila di livelli controlla la modalità di visualizzazione della pila di livelli. Poiché un livello può coprire più canali, non è possibile visualizzare tutte queste proprietà contemporaneamente. La modalità visualizzazione può quindi essere utilizzata per definire il contesto di visualizzazione corrente. Quando si utilizza questo menu a discesa è possibile specificare quali canali devono essere utilizzati per visualizzare nelle miniature dei livelli, nonché controllare il metodo di fusione e l’opacità solo per questo canale.
...
L’elenco di icone in alto a destra contiene le azioni più comuni che è possibile eseguire in Pila di livelli:
...
| Crea nuovo livello pittura <div><img class="" data-preserve-html="true" id="root_content_flex_items_position_position-par_dx_table_row-r3-column-c0_dynamic_grid_items_grid-cell_position-par_image" src="../../assets/image2020-9-30-11-52-41.png"/></div> | Create un nuovo livello di pittura sopra quello correntemente selezionato. |
...
| Aggiungi nuovi materiali avanzati <div><img class="" data-preserve-html="true" id="root_content_flex_items_position_position-par_dx_table_row-r5-column-c0_dynamic_grid_items_grid-cell_position-par_image" src="../../assets/icon-smartmat.png"/></div> | Inserite un nuovo materiale avanzato sopra il livello attualmente selezionato.Facendo clic su questo pulsante si aprirà un ripiano per sfogliare l&#39;elenco dei materiali avanzati disponibili nelle [risorse](../../interface/assets/assets.md) correnti. |
